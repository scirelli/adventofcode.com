...
function validPage(page) {
	for(let i=0,r; i<page.length-1; i++) {
		r = map[page[i]];
		if(!r) continue;
		for(let k=i+1; k<page.length; k++){
			if(r.indexOf(page[k]) !== -1) return false;
		}
	}
	return true;
}
...
function Node(v) {
	...
	this.insertBefore = function insertBefore(n) {
		if(this.before) {
			this.before.after = n;
	...
	this.insertAfter = function insertAfter(n) {
		if(this.after) {
			this.after.before = n;
			n.after = this.after;
		}
