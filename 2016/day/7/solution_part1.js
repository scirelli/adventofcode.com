...
function solve(input) {
    return input.reduce(function(sum, line){
        ...
        for(var i=0, l=a.length; i<l; i++){
            if(i%2){
                failed = failed || isPassingCase(a[i]);
            }else{
                passed = passed || isPassingCase(a[i]);
            }
        }
    ...
    function isPassingCase(str){
        for(var i=0, l=str.length-3,first,second,third,fourth; i<l; i++) {
            first = str.charAt(i);
            second = str.charAt(i+1);
            third = str.charAt(i+2);
            fourth = str.charAt(i+3);

            if(first !== third && (first+second) === (fourth+third)){
                return true;
            }
        }
        return false;
