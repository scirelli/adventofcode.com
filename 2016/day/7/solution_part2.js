...
function solve(input) {
    return input.reduce(function(sum, line){
        var valid = 0,
            aba = [],
            bab = [],
            a = line.split(/[\[\]]/);
        ...
        for(var i=0, l=a.length; i<l; i++){
            if(i%2){
                aba = aba.concat(getAllABA(a[i]));
            }else{
                bab = bab.concat(getAllABA(a[i]));
            }
        }
        
        aba = aba.reduce(function(acu, curValue){
             acu[curValue.charAt(1) + curValue.charAt(0) + curValue.charAt(1)] = true;
             return acu;
        }, {});

        for(var value in bab){
            if(aba[bab[value]]){
                return ++sum;
            }
        }
    ...
    function getAllABA(str){
        var rtn = [];

        for(var i=0, l=str.length-2,first,second,third; i<l; i++) {
            first = str.charAt(i);
            second = str.charAt(i+1);
            third = str.charAt(i+2);

            if(first !== second && first === third){
                rtn.push(first+second+third);
            }
        }
