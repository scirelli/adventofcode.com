...
instr = """value 23 goes to bot 208
...
def giveInitial():
	for line in inarr:
		s = line.split(' ')
		if(s[0]=='value'):
			give2bot(int(s[5]),int(s[1]))

def remove(subject,val):
	for i in range(0,len(bots[subject])):
		if(bots[subject][i]==val):
			bots[subject].pop(i)
			if len(bots[subject])==0:
				del bots[subject]
			return
...
def performBotFunction(subject,lowDest,lowType,highDest,highType):
	if subject not in bots or len(bots[subject])!=2: return 0
	valLow = min(bots[subject][0], bots[subject][1])
	valHigh = max(bots[subject][0], bots[subject][1])
	if valHigh==61 and valLow==17: print("FOUND: " + str(subject))
	give(subject,lowDest,lowType,valLow)
	give(subject,highDest,highType,valHigh)
	return 1
...
def runBotFunctions():
	...
	for line in inarr:
		s = line.split(' ')
		if s[0]=='bot':
			pr = performBotFunction(int(s[1]),int(s[6]),s[5],int(s[11]),s[10])
			if pr!=0:
				#print(line)
				p+=1
	return p
...
totalOper = 0
while(oper):
	oper = runBotFunctions()
	totalOper += oper
	print(bots)

#print('Total Oper: ' + str(totalOper))
print(bots)
