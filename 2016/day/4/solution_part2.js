...
totally-real-room-200[decoy]`
    exampleInput2 = `qzmt-zixmtkozy-ivhz-343[zimth]`,
    ALPHABET = 'abcdefghijklmnopqrstuvwxyz'.split('');

var list = parseInput(input)
              .filter(filterForValidRooms)
              .map(decryptName)
              .filter((o)=>{ 
                  return o.decryptedName.indexOf('north') > -1;
              });

console.log(list);

function decryptName(room){
    var rotatedAlphabet = ALPHABET.rotateRight(parseInt(room.serialNumber)),
        cypherAlphabet = rotatedAlphabet.reduce(function(accum, actual, index) {
                            var A_CHAR_CODE = 'a'.charCodeAt(0);
                            accum[actual] = String.fromCharCode(index + A_CHAR_CODE);
                            return accum;
                         }, {});
    
    room.decryptedName = '';
    for(var index=0, l=room.roomName.length, c; index<l; index++){
        c = room.roomName.charAt(index);
        if(c !== '-'){
            room.decryptedName += cypherAlphabet[c];
        }else{
            room.decryptedName += ' ';
        }
    }
...
Array.prototype.rotateRight = function(count){
    var self = this,
        rotatedArray = [];

    count = count || 1;
    count = count%self.length;
    for(var i=0, l=self.length, newPos; i<l; i++){
        newPos = (i+count)%self.length;
        rotatedArray[newPos] = self[i];
    }

    return rotatedArray;
};

function filterForValidRooms(room){
    var checksum;
  
    checksum = getChecksumFromSpecialOrderObject(getSpecialOrderObjectFreq(getCharFreq(room.encodedStr)));
...
function getChecksumFromSpecialOrderObject(specialOrderObject){
    var orderedCountList = Object.getOwnPropertyNames(specialOrderObject).map(Number).sort((a,b)=>{return b-a;}),
        checksum = '';

    for(var i=0, l=orderedCountList.length, index; i<l; i++){
        index = orderedCountList[i];
        checksum += specialOrderObject[index].join('');
    }
    return checksum.substr(0,5);
}
function getSpecialOrderObjectFreq(freq){
    var serialOrder = {},
        count, c,
        aCode = 'a'.charCodeAt(0);

    for(var charCode=0, l=freq.length; charCode<l; charCode++){
        count = freq[charCode];
        c = String.fromCharCode(charCode + aCode);
        if(count > 0){
            if(serialOrder[count]){
                serialOrder[count].push(c);
                serialOrder[count].sort();
            }else{
                serialOrder[count] = [c];
            }
        }
...
function getCharFreq(string){
    var freq = (new Array(26)).fill(0),
        aCode = 'a'.charCodeAt(0);

    for(var c in string){
        c = string.charCodeAt(c) - aCode;
        freq[c] = freq[c] + 1;
    }
    return freq;
}
function parseInput(input){
    return input.split('\n').map(function(encodedStr){
        var o = encodedStr.split('-'),
            serialAndCheckSum = o.pop(),
            serial, checksum, roomName;
        roomName = o.join('-');
        encodedStr = o.join('');
        serialAndCheckSum = serialAndCheckSum.split('[');
        serial = serialAndCheckSum[0];
        checksum = serialAndCheckSum[1].replace(']','');
